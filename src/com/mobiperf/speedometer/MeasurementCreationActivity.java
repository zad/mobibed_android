...
import com.mobiperf.speedometer.measurements.MobiBedTask;
import com.mobiperf.speedometer.measurements.PingTask;
import com.mobiperf.speedometer.measurements.MobiBedTask.MobiBedDesc;
import com.mobiperf.speedometer.measurements.PingTask.PingDesc;
import com.mobiperf.speedometer.measurements.TCPTask;
import com.mobiperf.speedometer.measurements.TCPTask.TCPTaskDesc;
  ...
  private class ButtonOnClickListener implements OnClickListener {
    ...
    public void onClick(View v) {
      ...
      try {
        ...
        if (measurementTypeUnderEdit.equals(MobiBedTask.TYPE)
                || measurementTypeUnderEdit.equals(Config.ALLTASK_TYPE)) {
        	BufferedReader reader = new BufferedReader(
        	        new InputStreamReader(getAssets().open(getString(R.string.default_script))));
        	StringBuilder script = new StringBuilder();
        	String line;
        	while((line = reader.readLine()) != null)
        		script.append(line + "\n");
        	Logger.d("run mobibed script: " + script.toString());
            MobiBedDesc desc = new MobiBedDesc(null, Calendar.getInstance().getTime(), null,
            		Config.DEFAULT_USER_MEASUREMENT_INTERVAL_SEC, Config.DEFAULT_USER_MEASUREMENT_COUNT,
            		MeasurementTask.USER_PRIORITY, null, script.toString(), null);
            newTask = new MobiBedTask(desc,
            		MeasurementCreationActivity.this.getApplicationContext());
        }
        
        if (measurementTypeUnderEdit.equals(TCPTask.TYPE)
                || measurementTypeUnderEdit.equals(Config.ALLTASK_TYPE)) {
        	Logger.d("run tcp task.");
        	TCPTaskDesc desc = new TCPTaskDesc(null, Calendar.getInstance().getTime(), null,
            		Config.DEFAULT_USER_MEASUREMENT_INTERVAL_SEC, Config.DEFAULT_USER_MEASUREMENT_COUNT,
            		MeasurementTask.USER_PRIORITY, null, null, null);
            newTask = new TCPTask(desc,
            		MeasurementCreationActivity.this.getApplicationContext());
        }
