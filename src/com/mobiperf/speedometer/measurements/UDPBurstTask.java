...
public class UDPBurstTask extends MeasurementTask {
  ...
  public static class UDPBurstDesc extends MeasurementDesc {
    ...
    public UDPBurstDesc(String key, Date startTime, Date endTime, double intervalSec, long count,
        long priority, Map<String, String> params) throws InvalidParameterException {
      super(UDPBurstTask.TYPE, key, startTime, endTime, intervalSec, 
    		  count, priority, params, null, null);
  ...
  public UDPBurstTask(MeasurementDesc desc, Context parent) {
    super(new UDPBurstDesc(desc.key, desc.startTime, desc.endTime, desc.intervalSec, desc.count,
        desc.priority, desc.parameters), parent);
  }
  ...
  public MeasurementTask clone() {
    ...
    UDPBurstDesc newDesc = new UDPBurstDesc(desc.key, desc.startTime, desc.endTime,
  ...
  private DatagramSocket sendUpBurst() throws MeasurementError {
    ...
    for (int i = 0; i < desc.udpBurstCount; i++) {
      ...
      // Log.i(SpeedometerApp.TAG, "Sent packet pnum:" + i + " to " +
      // desc.target + ": " + targetIp);
    } // for()
  ...
  private int recvUpResponse(DatagramSocket sock) throws MeasurementError {
    ...
    // Receive response
    // Log.i(SpeedometerApp.TAG, "Waiting for UDP response from " +
    // desc.target + ": " + targetIp);
    ...
    ByteArrayInputStream byteIn = new ByteArrayInputStream(recvpacket.getData(), 0,
    ...
    // Log.i(SpeedometerApp.TAG, "Recv UDP resp from " + desc.target +
    // " type:" + ptype + " burst:"
  ...
  private DatagramSocket sendDownRequest() throws MeasurementError {
    ...
    // Log.i(SpeedometerApp.TAG, "Requesting UDP burst:" +
    // desc.udpBurstCount + " pktsize: "
   ...
   * 
   * @param sock
   *          the datagram socket that can be used to receive the server's burst
   * 
  ...
  public MeasurementResult call() throws MeasurementError {
    ...
    MeasurementResult result = new MeasurementResult(phoneUtils.getDeviceInfo().deviceId,
        phoneUtils.getDeviceProperty(), UDPBurstTask.TYPE, System.currentTimeMillis() * 1000,
        isMeasurementSuccessful, this.measurementDesc);
