...
public class TracerouteTask extends MeasurementTask {
  ...
  public static class TracerouteDesc extends MeasurementDesc {
    ...
    // the total number of pings will send before we declarethe traceroute
    // fails
  ...
  public MeasurementResult call() throws MeasurementError {
    ...
    while (maxHopCount-- >= 0 && !stopRequested) {
      /*
       * Current traceroute implementation sends out three ICMP probes per TTL. One ping every 0.2s
       * is the lower bound before some platforms requires root to run ping. We ping once every time
       * to get a rough rtt as we cannot get the exact rtt from the output of the ping command with
       * ttl being set
       */
      String command = Util.constructCommand(task.pingExe, "-n", "-t", ttl, "-s",
          task.packetSizeByte, "-c 1 ", target);
      try {
        double rtt = 0;
        long t1;
        ...
        for (String ip : hostsAtThisDistance) {
          // If we have reached the final destination hostIp, print it
          // out and clean up
          if (ip.compareTo(hostIp) == 0) {
            ...
            // Otherwise, we aggregate various hosts at a given hop
            // distance for printout
